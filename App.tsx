...
const AppContent: React.FC = () => {
  ...
  // Seletor de Portais para usuários com múltiplos cargos
      ...
      return (
          <div className="fixed inset-0 z-50 bg-[#0f0f10] flex flex-col items-center justify-center p-4">
              <div className="max-w-5xl w-full text-center">
                  <img src="https://i.ibb.co/kgxf99k5/LOGOS-10-ANOS-BRANCA-E-VERMELHA.png" className="h-24 mx-auto mb-10"/>
                  <h2 className="text-4xl font-bold text-white mb-12 uppercase tracking-tight">Bem-vindo, {user.name.split(' ')[0]}</h2>
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-8 mb-12">
                      {user.roles.includes(UserRole.TEACHER) && (
                          <button onClick={() => setSessionRole(UserRole.TEACHER)} className="bg-white p-10 rounded-[2.5rem] flex flex-col items-center hover:scale-105 transition-all group border-4 border-transparent hover:border-red-600 shadow-2xl">
                              ...
                              <span className="font-black text-gray-800 uppercase text-sm">Portal do Professor</span>
                          </button>
                      )}
                      {user.roles.includes(UserRole.PRINTSHOP) && (
                          <button onClick={() => setSessionRole(UserRole.PRINTSHOP)} className="bg-white p-10 rounded-[2.5rem] flex flex-col items-center hover:scale-105 transition-all group border-4 border-transparent hover:border-blue-600 shadow-2xl">
                              ...
                              <span className="font-black text-gray-800 uppercase text-sm">Painel Gestão Escolar</span>
                          </button>
                      )}
                      {user.roles.includes(UserRole.AEE) && (
                          <button onClick={() => setSessionRole(UserRole.AEE)} className="bg-white p-10 rounded-[2.5rem] flex flex-col items-center hover:scale-105 transition-all group border-4 border-transparent hover:border-pink-600 shadow-2xl">
                              ...
                              <span className="font-black text-gray-800 uppercase text-sm">Portal AEE</span>
                          </button>
                      )}
                  </div>
                  <button onClick={logout} className="text-gray-500 hover:text-white flex items-center gap-2 mx-auto uppercase text-xs font-black tracking-widest transition-colors"><UserCircle2 size={18}/> Sair do Sistema</button>
              </div>
          </div>
      );
  }
    ...
    <div className="h-screen flex flex-col bg-[#0f0f10] text-gray-100 font-sans overflow-hidden">
        ...
        <div className="w-full mx-auto px-6 flex justify-between h-16 items-center">
            <div className="flex items-center gap-4">
              ...
              <span className="text-[10px] font-black text-gray-400 uppercase tracking-[0.2em] hidden md:block">
                {activeRole === UserRole.TEACHER ? 'Ambiente Docente' : 'Painel de Controle Administrativo'}
              </span>
            </div>
            <div className="flex items-center gap-4">
               {user?.roles && user.roles.length > 1 && (
                   <button onClick={() => setSessionRole(null)} className="p-2 text-gray-400 hover:text-white transition-colors" title="Trocar de Portal"><LayoutGrid size={20}/></button>
               )}
               <button onClick={() => setShowPasswordModal(true)} className="p-2 text-gray-400 hover:text-white transition-colors" title="Alterar Senha"><KeyRound size={20}/></button>
               <button onClick={logout} className="p-2 text-gray-400 hover:text-red-400 transition-colors" title="Sair"><LogOut size={20}/></button>
            </div>
        </div>
      </nav>
      ...
      {showPasswordModal && (
          <div className="fixed inset-0 z-[110] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
              <div className="bg-[#18181b] border border-gray-800 w-full max-w-md rounded-[2rem] p-8 shadow-2xl">
                  <h3 className="text-xl font-black text-white mb-6 uppercase tracking-tight">Redefinir Senha</h3>
                  <form onSubmit={handleChangePassword} className="space-y-4">
                      <input type="password" placeholder="Nova Senha" className="w-full bg-black/30 border border-gray-700 rounded-xl p-3 text-white focus:border-red-600 outline-none" value={newPassword} onChange={e => setNewPassword(e.target.value)} required />
                      <input type="password" placeholder="Confirmar Nova Senha" className="w-full bg-black/30 border border-gray-700 rounded-xl p-3 text-white focus:border-red-600 outline-none" value={confirmPassword} onChange={e => setConfirmPassword(e.target.value)} required />
                      {passwordError && <p className="text-red-500 text-xs font-bold uppercase">{passwordError}</p>}
                      <div className="flex justify-end gap-3 pt-4"><Button type="submit" isLoading={isChangingPassword} className="w-full h-12 rounded-xl font-black uppercase text-xs">Atualizar Agora</Button></div>
                  </form>
              </div>
          </div>
      )}
