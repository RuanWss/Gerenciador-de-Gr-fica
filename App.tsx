...
import { LogOut, LayoutGrid, KeyRound, GraduationCap, School, Heart, UserCircle2, Megaphone, Book, AlertCircle } from 'lucide-react';
...
const AppContent: React.FC = () => {
      ...
      return (
          <div className="fixed inset-0 z-50 bg-[#0f0f10] flex flex-col items-center justify-center p-4">
              <div className="max-w-5xl w-full text-center">
                  <img src="https://i.ibb.co/kgxf99k5/LOGOS-10-ANOS-BRANCA-E-VERMELHA.png" className="h-24 mx-auto mb-6"/>
                  <h2 className="text-4xl font-bold text-white mb-8 uppercase tracking-tight">Bem-vindo, {user.name.split(' ')[0]}</h2>
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 mb-12">
                      {user.roles.includes(UserRole.TEACHER) && (
                          <button onClick={() => setSessionRole(UserRole.TEACHER)} className="bg-white p-8 rounded-3xl flex flex-col items-center hover:scale-105 transition-all group border-4 border-transparent hover:border-red-600">
                              <GraduationCap size={48} className="text-red-600 mb-4" />
                              <span className="font-bold text-gray-800 uppercase text-sm">Portal do Professor</span>
                          </button>
                      )}
                      {user.roles.includes(UserRole.PRINTSHOP) && (
                          <button onClick={() => setSessionRole(UserRole.PRINTSHOP)} className="bg-white p-8 rounded-3xl flex flex-col items-center hover:scale-105 transition-all group border-4 border-transparent hover:border-blue-600">
                              <School size={48} className="text-blue-600 mb-4" />
                              <span className="font-bold text-gray-800 uppercase text-sm">Painel da Escola</span>
                          </button>
                      )}
                      {user.roles.includes(UserRole.AEE) && (
                          <button onClick={() => setSessionRole(UserRole.AEE)} className="bg-white p-8 rounded-3xl flex flex-col items-center hover:scale-105 transition-all group border-4 border-transparent hover:border-pink-600">
                              <Heart size={48} className="text-pink-600 mb-4" />
                              <span className="font-bold text-gray-800 uppercase text-sm">Portal AEE</span>
                          </button>
                      )}
                  </div>
                  <button onClick={logout} className="text-gray-500 hover:text-white flex items-center gap-2 mx-auto"><UserCircle2 size={18}/> Sair</button>
              </div>
          </div>
      );
  }
  ...
  return (
    <div className="h-screen flex flex-col bg-transparent text-gray-100 font-sans overflow-hidden">
      ...
      <nav className="bg-black/40 backdrop-blur-md border-b border-white/10 shrink-0">
        <div className="w-full mx-auto px-6 flex justify-between h-16 items-center">
            <div className="flex items-center gap-4">
              ...
              <span className="text-sm font-bold text-gray-300 uppercase tracking-widest hidden md:block">
                {activeRole === UserRole.TEACHER ? 'Portal do Professor' : 'Painel de Gestão Escola'}
              </span>
            </div>
            <div className="flex items-center gap-4">
               {user?.roles && user.roles.length > 1 && (
                   <button onClick={() => setSessionRole(null)} className="p-2 text-gray-400 hover:text-white"><LayoutGrid size={18}/></button>
               )}
               <button onClick={() => setShowPasswordModal(true)} className="p-2 text-gray-400 hover:text-white"><KeyRound size={18}/></button>
          ...
          <div className="fixed inset-0 z-[110] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4">
              <div className="bg-[#18181b] border border-gray-800 w-full max-w-md rounded-2xl p-6">
                  ...
                  <form onSubmit={handleChangePassword} className="space-y-4">
                      ...
                      <div className="flex justify-end gap-3 pt-2"><Button type="submit">Salvar</Button></div>
                  </form>
              </div>
          </div>
      )}
