...
async function genneraRequest(endpoint: string) {
  ...
  try {
    ...
    if (!response.ok) {
        throw { status: response.status, message: `Erro ${response.status} na API Gennera em ${path}` };
    }
...
/**
 * Retorna os alunos de uma turma conforme confirmado na imagem do usuário
 */
export const fetchGenneraStudentsByClass = async (classId: string, className: string): Promise<Student[]> => {
  ...
  return list.map((a: any) => ({
    ...
    name: String(a.studentName || a.nome || a.nome_pessoa || a.name || 'ALUNO SEM NOME').toUpperCase(),
    classId,
    className,
