...
// URL gerada pelo seu comando de deploy no Cloud Run do PROXY
const CLOUD_RUN_GATEWAY = 'https://cors-proxy-376976972882.europe-west1.run.app'; 
...
async function genneraRequest(endpoint: string, method: string = 'GET') {
  ...
  try {
    const response = await fetch(CLOUD_RUN_GATEWAY, {
      method: 'POST', // O Proxy sempre recebe POST para carregar os metadados
      headers: {
    ...
    console.error("Falha na integração Gennera:", error.message);
    throw error;
  }
}
