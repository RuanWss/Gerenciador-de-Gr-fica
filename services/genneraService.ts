...
async function genneraRequest(endpoint: string) {
  // Limpa o endpoint para garantir que começa com uma única barra
  ...
  try {
    ...
    if (!response.ok) {
        console.error(`Gennera API Error: ${response.status} em ${cleanEndpoint}`);
        throw new Error(`Erro ${response.status} na API Gennera`);
    }
    
    return await response.json();
  } catch (error: any) {
    console.error("Erro Gennera:", error.message);
    throw error;
  }
}
...
/**
 * Retorna as turmas.
 * Na v1 o endpoint de listagem de turmas da instituição é /institutions/{id}/classes
 */
export const fetchGenneraClasses = async (): Promise<SchoolClass[]> => {
  ...
  // Trata múltiplos formatos de retorno do ERP
...
/**
 * Retorna os alunos de uma turma conforme solicitado:
 * /institutions/:idInstitution/classes/:idClass/students
 */
