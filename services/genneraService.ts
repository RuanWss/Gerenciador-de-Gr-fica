...
/**
 * Usamos um caminho relativo. 
 * No desenvolvimento, o Vite Proxy intercepta.
 * Na produção, o server.js (Express) intercepta.
 */
...
async function genneraRequest(endpoint: string, method: string = 'GET') {
  // O endpoint já deve vir com a barra inicial, ex: /institutions/...
  ...
  try {
    ...
    if (!response.ok) {
        throw new Error(`Erro ${response.status} na integração direta.`);
    }
    
    return await response.json();
  } catch (error: any) {
    console.error("Falha Gennera Service:", error.message);
    throw error;
  }
}
