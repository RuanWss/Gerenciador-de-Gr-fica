...
// URL do Proxy no Cloud Run para evitar erros de CORS
...
async function genneraRequest(endpoint: string) {
  ...
  try {
    ...
    // Se falhar o fetch no proxy, pode ser rede ou proxy off
    if (error.message.includes('Failed to fetch')) {
        throw new Error("Não foi possível conectar ao serviço de sincronização (Proxy). Verifique sua internet ou tente novamente mais tarde.");
    }
    throw error;
  }
}
