...
// ID da instituição fornecido pelo usuário
const INSTITUTION_ID = '891';

// Token de API Gennera fornecido
...
export const fetchGenneraClasses = async (): Promise<SchoolClass[]> => {
  ...
  return list.map((t: any) => ({
    id: String(t.id || t.idClass),
    name: String(t.name || t.description).toUpperCase(),
    shift: String(t.shift || '').toLowerCase().includes('manhã') ? 'morning' : 'afternoon'
  }));
};

export const fetchGenneraStudentsByClass = async (classId: string, className: string): Promise<Student[]> => {
  // Caminho exato solicitado: institutions/{idInstitution}/classes/{idClass}/students
  ...
  return list.map((a: any) => ({
    ...
    name: String(a.nome_pessoa || a.nome || a.studentName).toUpperCase(),
    classId,
    className,
