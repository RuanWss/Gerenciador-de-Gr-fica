...
export const TeacherDashboard: React.FC = () => {
  ...
  const handleSaveMaterial = async () => {
    if (!materialTitle || !materialClass || !materialFile) return alert("Preencha todos os campos e anexe o arquivo.");
    setIsSaving(true);
    try {
        const fileUrl = await uploadExamFile(materialFile, user?.name || 'Professor');
        ...
        fetchData();
    } catch (e) { alert("Erro ao enviar material."); }
                                ...
                                <div className="space-y-12 animate-in fade-in duration-500">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                            {["Consciência ambiental/consumo responsável", "Criatividade e autoria (criar algo)", "Colaboração e protagonismo", "Comunicação (apresentar/explicar)", "Investigação (observação/pesquisa/dados)", "Uso responsável de tecnologia/IA"].map(opt => (
                                                <label key={opt} className={`flex items-center gap-5 p-6 rounded-3xl border cursor-pointer transition-all ${planData.expectedResults?.includes(opt) ? 'bg-red-600 border-red-500 shadow-xl' : 'bg-black/40 border-white/5 hover:border-white/20'}`} onClick={() => toggleSelection('expectedResults', opt)}>
                                                    <div className={`h-6 w-6 rounded-lg border flex items-center justify-center ${planData.expectedResults?.includes(opt) ? 'bg-white border-white' : 'border-gray-600'}`}>{planData.expectedResults?.includes(opt) && <X size={16} className="text-red-600"/>}</div>
                                    ...
                                    <section className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                        <div className="space-y-8">
                                            ...
                                            <div className="space-y-4">
                                                {["1. Sensibilizar", "2. Investigar", "3. Criar", "4. Testar e melhorar", "5. Apresentar", "6. Registrar"].map(step => (
                                                    <label key={step} className="flex items-center gap-4 p-5 bg-black/20 rounded-[1.5rem] border border-white/5 cursor-pointer group hover:bg-black/40 transition-colors" onClick={() => toggleSelection('projectSteps', step)}>
                                                        <div className={`h-7 w-7 rounded-xl border flex items-center justify-center transition-all ${planData.projectSteps?.includes(step) ? 'bg-red-600 border-red-500 shadow-lg' : 'border-gray-700'}`}>{planData.projectSteps?.includes(step) && <CheckCircle2 size={18} className="text-white"/>}</div>
                                    ...
                                    <section className="bg-red-900/5 p-10 rounded-[3.5rem] border border-red-500/10 space-y-10">
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <input className="w-full bg-black/40 border border-white/10 rounded-[1.5rem] p-6 text-white font-bold outline-none focus:border-red-600" value={planData.aiCareTaken} onChange={e => setPlanData({...planData, aiCareTaken: e.target.value})} placeholder="Ex: Verificação de fontes..." />
                                        </div>
                                    </section>
                                    <section className="space-y-8">
            ...
            {activeTab === 'materials' && (
                <div className="animate-in fade-in slide-in-from-right-4 max-w-6xl mx-auto pb-40">
                    <header className="mb-12">
                        ...
                        <p className="text-gray-400 font-bold uppercase text-[10px] tracking-widest">Enviar arquivos PDF para visualização em sala</p>
                    </header>
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
                        ...
                        <div className="bg-[#18181b] border-2 border-white/5 p-10 rounded-[3rem] shadow-2xl space-y-8">
                            <div className="flex items-center gap-4 mb-4">
                                <div className="h-12 w-12 bg-red-600/10 text-red-500 rounded-xl flex items-center justify-center"><UploadCloud size={24}/></div>
                            ...
                            <div className="grid grid-cols-2 gap-6">
                                <div className="space-y-4">
                                    <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest ml-2">Turma Destino</label>
                                    <select className="w-full bg-black/40 border border-white/10 p-5 rounded-2xl text-white font-bold outline-none appearance-none focus:border-red-600 transition-all" value={materialClass} onChange={e => setMaterialClass(e.target.value)}>
                                        <option value="">-- Turma --</option>
                                        {CLASSES.map(c => <option key={c} value={c}>{c}</option>)}
                                    </select>
                                </div>
                                <div className="space-y-4">
                                    <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest ml-2">Disciplina</label>
                                    <select className="w-full bg-black/40 border border-white/10 p-5 rounded-2xl text-white font-bold outline-none appearance-none focus:border-red-600 transition-all" value={materialSubject} onChange={e => setMaterialSubject(e.target.value)}>
                            ...
                            <div className="space-y-4">
                                <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest ml-2">Arquivo (PDF recomendado)</label>
                                <div className="border-3 border-dashed border-white/10 rounded-[2rem] p-10 text-center hover:border-red-600 transition-all relative bg-black/20 group cursor-pointer">
                                    ...
                                    {materialFile ? (
                                        <div className="text-red-500 flex flex-col items-center animate-in zoom-in-95">
                                            <CheckCircle2 size={48} className="mb-2" />
                                            <span className="font-bold text-sm truncate max-w-[200px]">{materialFile.name}</span>
                                        </div>
                                    ) : (
                                        <>
                                ...
                                {teacherMaterials.length > 0 ? teacherMaterials.map(mat => (
                                    <div key={mat.id} className="bg-[#18181b] border border-white/5 p-6 rounded-3xl flex justify-between items-center group hover:border-red-600/30 transition-all">
                                        <div className="flex items-center gap-5">
                                            <div className="p-3 bg-red-600/10 text-red-500 rounded-xl"><FileText size={28}/></div>
                                            <div>
                                                <h4 className="font-black text-white uppercase tracking-tight text-sm leading-tight mb-1">{mat.title}</h4>
                                                <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">{mat.className} • {mat.subject}</p>
                                            </div>
                                        </div>
                                    ...
                                    <div className="py-20 text-center opacity-30 font-black uppercase tracking-[0.3em] text-sm text-gray-600 bg-white/5 rounded-[2.5rem] border-2 border-dashed border-white/5">Nenhum material enviado</div>
                                )}
                            </div>
                        </div>
                    </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/95 backdrop-blur-xl">
                    <div className="bg-[#18181b] border border-white/10 w-full max-w-5xl max-h-[90vh] rounded-[3.5rem] shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95">
                        <div className="p-10 border-b border-white/5 bg-red-600/5 flex items-center justify-between shrink-0">
                            <div className="flex items-center gap-8">
                                <div className="h-20 w-20 bg-red-600 rounded-[2rem] flex items-center justify-center shadow-2xl shadow-red-900/40"><Heart size={38}/></div>
                                <div><h2 className="text-3xl font-black uppercase tracking-tight">Elaboração de Documento PEI</h2><p className="text-gray-400 font-bold uppercase text-[10px] tracking-[0.3em]">Adaptação Pedagógica Individualizada</p></div>
                            </div>
            ...
            {activeTab === 'occurrences' && (
                <div className="animate-in fade-in slide-in-from-right-4 pb-40">
                    <header className="mb-12 flex justify-between items-center">
                        <div><h1 className="text-4xl font-black uppercase tracking-tighter leading-tight">Minhas Ocorrências</h1><p className="text-gray-400 font-bold uppercase text-[10px] tracking-widest">Relato de eventos e comportamentos em sala.</p></div>
                        <Button onClick={() => setShowOccForm(true)} className="bg-red-600 h-16 px-10 rounded-[2rem] font-black uppercase text-xs tracking-widest shadow-2xl shadow-red-900/40"><PlusCircle size={20} className="mr-3"/> Nova Ocorrência</Button>
                    </header>
