...
export const TeacherDashboard: React.FC = () => {
  ...
  const finalizeExam = async () => {
      ...
      if (creationMode === 'upload' && !uploadedFile) return alert("Selecione um arquivo.");
      
      setIsSaving(true);
      try {
          let fileUrl = '';
          let fileName = 'request_header.pdf';
  ...
  const saveMaterial = async () => {
      if (!matTitle || !matGrade || !matSubject || !matFile) return alert("Preencha todos os campos e o arquivo.");
      ...
      try {
          ...
          alert("Material publicado na sala!");
          resetMaterialForm();
          fetchData();
      } catch (e) {
          console.error(e);
    ...
    <div className="flex h-[calc(100vh-80px)] overflow-hidden -m-8 bg-transparent">
        ...
        <div className="w-64 bg-black/20 backdrop-blur-xl border-r border-white/10 p-6 flex flex-col h-full z-20 shadow-2xl">
            <div className="mb-6">
                ...
                <button onClick={() => setActiveTab('requests')} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl mb-1 text-sm font-medium transition-all ${activeTab === 'requests' ? 'bg-red-600 text-white shadow-lg shadow-red-900/40' : 'text-gray-300 hover:bg-white/10'}`}><List size={18} /> Pedidos Gráfica</button>
                <button onClick={() => { setCreationMode('none'); setActiveTab('create'); }} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl mb-1 text-sm font-medium transition-all ${activeTab === 'create' ? 'bg-red-600 text-white shadow-lg shadow-red-900/40' : 'text-gray-300 hover:bg-white/10'}`}><PlusCircle size={18} /> Novo Pedido</button>
            ...
            {activeTab === 'requests' && (
                <div className="animate-in fade-in slide-in-from-right-4">
                    <header className="mb-8">
                        <h1 className="text-3xl font-black text-white uppercase tracking-tighter">Status da Gráfica</h1>
                        <p className="text-gray-400">Acompanhe suas impressões solicitadas.</p>
                    </header>
                                        ...
                                        <td className="p-6">
                                            <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest ${
                                                ...
                                                {e.status === ExamStatus.PENDING ? 'Pendente' : 
                                                 e.status === ExamStatus.IN_PROGRESS ? 'Em Fila' : 'Pronto'}
                                            </span>
                                        </td>
                                    </tr>
                                ))}
                    ...
                    {creationMode === 'none' ? (
                        <div className="max-w-4xl mx-auto py-12">
                             <div className="text-center mb-12">
                                <h1 className="text-4xl font-black text-white uppercase tracking-tight mb-2">Enviar Impressão</h1>
                                <p className="text-gray-400 text-lg">Selecione o modo de envio para a gráfica:</p>
                             </div>
                             <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <button onClick={() => setCreationMode('create')} className="bg-[#18181b] border-4 border-white/5 p-12 rounded-[3rem] text-center hover:scale-105 hover:border-red-600 transition-all group">
                                    ...
                                    <h3 className="text-2xl font-black text-white uppercase mb-4">Gerar Cabeçalho</h3>
                                ...
                                <button onClick={() => setCreationMode('upload')} className="bg-[#18181b] border-4 border-white/5 p-12 rounded-[3rem] text-center hover:scale-105 hover:border-blue-600 transition-all group">
                                    ...
                                    <h3 className="text-2xl font-black text-white uppercase mb-4">Enviar Qualquer Arquivo</h3>
                                    <p className="text-gray-500">Upload de PDF, Word, Imagens, Excel ou qualquer outro formato.</p>
                                </button>
                             </div>
                        </div>
                    ) : (
                            ...
                            <div className="bg-[#18181b] rounded-[2.5rem] border border-white/10 p-10 shadow-2xl relative overflow-hidden">
                                <div className="flex items-center justify-between mb-10 relative z-10">
                                    <h3 className="text-2xl font-black text-white uppercase flex items-center gap-4">
                                        ...
                                        {creationMode === 'upload' ? 'Upload de Material' : 'Pedido de Cabeçalho'}
                                    </h3>
                                    <button onClick={() => { setCreationMode('none'); setUploadedFile(null); }} className="text-gray-400 hover:text-white transition-colors"><X size={32}/></button>
                                </div>

                                <div className="space-y-6 relative z-10">
                                    <div>
                                        <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2 ml-1">Título do Trabalho / Prova</label>
                                    ...
                                    <div className="grid grid-cols-2 gap-6">
                                        ...
                                        <div>
                                            <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2 ml-1">Qtd. Cópias</label>
                                    ...
                                    <div>
                                        <label className="block text-[10px] font-black text-gray-500 uppercase tracking-widest mb-2 ml-1">Instruções de Impressão</label>
                                        <textarea className="w-full bg-black/40 border border-white/10 rounded-2xl p-4 text-white font-bold outline-none focus:border-red-600 transition-all" rows={3} value={examInstructions} onChange={e => setExamInstructions(e.target.value)} placeholder="Ex: Frente e verso, grampeado, colorido..."></textarea>
                                    </div>
                ...
                <div className="animate-in fade-in slide-in-from-right-4">
                    <header className="mb-8 flex justify-between items-center">
                        <div>
                            ...
                            <p className="text-gray-400">Arquivos compartilhados para as salas de aula.</p>
                        </div>
                    </header>
                            ...
                            <div className="bg-[#18181b] border border-white/10 rounded-[2rem] p-8 shadow-xl sticky top-0">
                                ...
                                <div className="space-y-4">
                                    <div>
                                        ...
                                        <input className="w-full bg-black/40 border border-white/10 rounded-xl p-3 text-white text-sm outline-none focus:border-red-600" value={matTitle} onChange={e => setMatTitle(e.target.value)} placeholder="Ex: Slide Aula 05 - Revolução" />
                                    </div>
                                    <div>
